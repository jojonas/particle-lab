...
The time resolution of the full setup can now be obtained from a fitted gauss function. The fit shows deviations to larger delay times (asymmetry in the tail) and the $\chi^2$ value does not match the expectation. However, in 


\begin{equation}
	\sigma_{t,full}= \left(  \right) \si{\pico\second}
\end{equation}

\section{Short Positron Lifetime in Aluminum}

\section{Long Positron Lifetime in Plastic}
