...
\label{eq:E_res}
\end{equation}
To estimate the impact of the limited resolution, the template fits are repeated 10 times but each event is smeared randomly following a gaussian distribution with $\sigma$ from equation \ref{eq:E_res}. This procedure results in 10 fitted \PW boson masses. The variance of this set is then taken as an estimator for the systematic uncertainty due to the energy resolution. The result is stated in table \ref{tbl:systematics}.
...
\begin{table}[htbp]
	...
	\begin{tabular}{ 
		...
		\midrule
		Transverse Mass \MT & 0.018 &   \\
		Transverse Electron Energy & 0.045 &  \\
		\bottomrule
	\end{tabular}
